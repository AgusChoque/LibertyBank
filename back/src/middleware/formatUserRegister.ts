...
const formatUserRegister = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(nameSplit.length > 50 || nameSplit.length < 10) next({statusCode: 400, message: "The name must be between 10 and 50 characters."});
    ...
    if(!Number.isInteger(year)) next({statusCode: 400, message: "The year must be an integer."});
    if(year > 2013 || year < 1925) next({statusCode: 400, message: "The year must be between 1925 and 2013."});
    if(!Number.isInteger(month)) next({statusCode: 400, message: "The month must be an integer."});
    if(month < 1 || month > 12) next({statusCode: 400, message: "The month must be between 1 and 12."});
    if(!Number.isInteger(day)) next({statusCode: 400, message: "The day must be an integer."});
    ...
    if (password.length < 8) next({statusCode: 400, message: "The username mustn't be less than 8 characters."});
